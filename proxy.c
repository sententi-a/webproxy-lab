...
// Web content in memory cache, Shared by all connections
cache_t cache;
...
int main(int argc, char **argv) 
{
    ...
    // Yep. Not using the same approach we did in concurrent echoserver.
    int connfd;
    socklen_t clientlen;
...
void doit(int connfd) 
{
    ...
    if (!Rio_readlineb(&conn_rio, buf, MAXLINE)) { return; }
...
// Shutdown Handling - Signal
void sigint_handler(int sig) 
{
    int olderrno = errno;

    Sio_puts("Shutting Down ..\nFreeing allocated memory...\n");
